...
echo -e "ESTATICOS + DINAMICOS: juntando en un CSV Ãºnico..." >> ${LOG_MASTER}
...
# PENDIENTE: de momento, no limpiamos, pero habrÃ¡ que hacerlo
...
do
	...
	echo -e "Capa 6 - PASADO Ã³ FUTURO: balancea las clases (aunque ya se hizo en capa 5), divide dataset de entrada (entrenamiento, test, validaciÃ³n), CREA MODELOS (con hyperparÃ¡metros)  los evalÃºa. Guarda el modelo GANADOR de cada subgrupo..." >> ${LOG_MASTER}
	python3 "${PYTHON_SCRIPTS}bolsa/C6CreadorModelosDeSubgrupo.py" "${dir_subgrupo}/" "${DIR_TIEMPO}"  >> ${LOG_MASTER}
	
	echo -e "Validaci�n de rendimiento..." >> ${LOG_MASTER}
	java -Djava.util.logging.SimpleFormatter.format="%1$tY-%1$tm-%1$td %1$tH:%1$tM:%1$tS %4$s %2$s %5$s%6$s%n" -jar ${PATH_JAR} --class "coordinador.Principal" "c70X.validacion.Validador" "${PATH_PREDICHO}" "${PATH_VALIDACION}" "${S}" "${X}" "${R}" "${M}" 2>>${LOG_MASTER} 1>>${LOG_MASTER}
