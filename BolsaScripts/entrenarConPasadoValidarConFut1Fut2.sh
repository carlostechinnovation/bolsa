...
if [ "${ACTIVAR_DESCARGAS}" = "N" ];  then
	echo -e "PASADO - Usamos datos LOCALES (sin Internet) de la ruta ${DIR_BASE}datos_validacion/pasado_brutos_csv/" >>${LOG_VALIDADOR}
	...
	if [ ${tamanioorigen} > 0 ]; then
		...
		cp -a "${DIR_BASE}validacion/pasado_brutos_csv/." "${DIR_BASE}pasado/brutos_csv/" >>${LOG_VALIDADOR}
	else
		echo "No existe o no hay datos en: ${DIR_BASE}validacion/pasado_brutos_csv/   Saliendo..."
		exit -1
	fi
	...
	rm -Rf ${DIR_BASE}pasado/ >>${LOG_VALIDADOR}
	mkdir -p ${DIR_BASE}logs/ >>${LOG_VALIDADOR}
	echo "Se borra y se crea TOTALMENTE la carpeta:  ${DIR_BASE}pasado/"
	echo -e ""  >>${LOG_VALIDADOR}
fi
...
if [ "${ACTIVAR_DESCARGAS}" = "N" ];  then
	...
	echo -e "FUTURO1 - Usamos datos LOCALES (sin Internet) de la ruta ${DIR_BASE}datos_validacion/futuro1_brutos_csv/" >>${LOG_VALIDADOR}
	tamanioorigen=$(du -s "${DIR_BASE}validacion/futuro1_brutos_csv/" | cut -f1)
	echo "${tamanioorigen}"
	
	if [ ${tamanioorigen} > 0 ]; then
		rm -Rf ${DIR_BASE}futuro/ >>${LOG_VALIDADOR}
		crearCarpetaSiNoExiste "${DIR_FUT_SUBGRUPOS}"
		crearCarpetaSiNoExiste "${DIR_BASE}futuro/brutos_csv/"
		cp -a "${DIR_BASE}validacion/futuro1_brutos_csv/." "${DIR_BASE}futuro/brutos_csv/" >>${LOG_VALIDADOR}
	else
		echo "No existe o no hay datos en: ${DIR_BASE}validacion/futuro1_brutos_csv/   Saliendo..."
		exit -1
	fi
	
else
	echo "Se borra y se crea TOTALMENTE la carpeta:  ${DIR_BASE}futuro/"
	rm -Rf ${DIR_BASE}futuro/ >>${LOG_VALIDADOR}
	crearCarpetaSiNoExiste "${DIR_FUT_SUBGRUPOS}"
fi
...
${PATH_SCRIPTS}master.sh "futuro" "${FUTURO1_t1}" "1" "${ACTIVAR_DESCARGAS}" "S" "${S}" "${X}" "${R}" "${M}" "${F}" "${B}" "${NUM_EMPRESAS}" "${UMBRAL_SUBIDA_POR_VELA}" "${UMBRAL_MINIMO_GRAN_VELA}" "${MIN_COBERTURA_CLUSTER}" "${MIN_EMPRESAS_POR_CLUSTER}" "20001111" "20991111" "${MAX_NUM_FEAT_REDUCIDAS}" "${CAPA5_MAX_FILAS_ENTRADA}" "${DINAMICA1}" "${DINAMICA2}" 2>>${LOG_VALIDADOR} 1>>${LOG_VALIDADOR}
...
dir_val_futuro_1="${DIR_BASE}validacion/E${NUM_EMPRESAS}_VR${VELAS_RETROCESO}_S${S}_X${X}_R${R}_M${M}_F${F}_B${B}_futuro1/"
rm -Rf $dir_val_futuro_1
mkdir -p $dir_val_futuro_1
cp -R "${DIR_BASE}futuro/" $dir_val_futuro_1
...
if [ "${ACTIVAR_DESCARGAS}" = "N" ];  then
	...
	echo -e "FUTURO2 - Usamos datos LOCALES (sin Internet) de la ruta ${DIR_BASE}datos_validacion/futuro2_brutos_csv/" >>${LOG_VALIDADOR}
	tamanioorigen=$(du -s "${DIR_BASE}validacion/futuro2_brutos_csv/" | cut -f1)
	echo "${tamanioorigen}"
	...
	if [ ${tamanioorigen} > 0 ]; then
		...
		crearCarpetaSiNoExiste "${DIR_FUT_SUBGRUPOS}"
		crearCarpetaSiNoExiste "${DIR_BASE}futuro/brutos_csv/"
		cp -a "${DIR_BASE}validacion/futuro2_brutos_csv/." "${DIR_BASE}futuro/brutos_csv/" >>${LOG_VALIDADOR}
	else
		echo "No existe o no hay datos en: ${DIR_BASE}validacion/futuro2_brutos_csv/   Saliendo..."
		exit -1
	fi
...
${PATH_SCRIPTS}master.sh "futuro" "${FUTURO2_t1}" "1" "${ACTIVAR_DESCARGAS}" "S" "${S}" "${X}" "${R}" "${M}" "${F}" "${B}" "${NUM_EMPRESAS}" "${UMBRAL_SUBIDA_POR_VELA}" "${UMBRAL_MINIMO_GRAN_VELA}" "${MIN_COBERTURA_CLUSTER}" "${MIN_EMPRESAS_POR_CLUSTER}" "20001111" "20991111" "${MAX_NUM_FEAT_REDUCIDAS}" "${CAPA5_MAX_FILAS_ENTRADA}" "${DINAMICA1}" "${DINAMICA2}" 2>>${LOG_VALIDADOR} 1>>${LOG_VALIDADOR}
...
dir_val_futuro_2="${DIR_BASE}validacion/E${NUM_EMPRESAS}_VR${VELAS_RETROCESO}_S${S}_X${X}_R${R}_M${M}_F${F}_B${B}_futuro2/"
rm -Rf $dir_val_futuro_2
mkdir -p $dir_val_futuro_2
cp -R "${DIR_BASE}futuro/" $dir_val_futuro_2
...
dir_val_logs="${DIR_BASE}validacion/E${NUM_EMPRESAS}_VR${VELAS_RETROCESO}_S${S}_X${X}_R${R}_M${M}_F${F}_B${B}_umbral${UMBRAL_SUBIDA_POR_VELA}_${DINAMICA1}_${DINAMICA2}_log/"
rm -Rf $dir_val_logs
mkdir -p $dir_val_logs
cp -R "${DIR_BASE}logs/" $dir_val_logs
