...
print("dirDropbox=" + dirDropbox)
print("dirAnalisis=" + dirAnalisis)
print("S=" + S)
print("X=" + X)
print("R=" + R)
print("M=" + M)
...
def pintar(resultadoAnalisis, subgrupos, Y):
    if not resultadoAnalisis.empty:
        # Se pintan en dos gráficas: precisión media y rentabilidad media ESPERANDO "Y" DIAS
        ...
        for subgrupo in subgrupos:
            ...
            if numeroDiasAnalizados > 0:
                ...
                resultadoPorSubgrupo.plot(kind='line', x='fecha', y='precisionMedia', ax=ax,
                                          label=subgrupo + " --> " + '{:.0f}%'.format(100 * media) + ', {:.0f}'.format(
                                              numeroDiasAnalizados) + ' compras', marker="+")
        ...
        for subgrupo in subgrupos:
            ...
            resultadoPorSubgrupo.plot(kind='line', x='fecha', y='rentaMedia', ax=ax2,
                                      label=subgrupo + " --> " + '{:.1f}%'.format(media) + ', {:.0f}'.format(
                                          numeroDiasAnalizados) + ' compras, ' + 'std: {:.1f}%'.format(std), marker="+")
        ...
        for subgrupo in subgrupos:
            ...
            if numeroDiasAnalizados > 0:
                ...
                resultadoPorSubgrupo.plot(kind='line', x='fecha', y='rentaAcumulada', ax=ax3,
                                          label=subgrupo + " --> " + '{:.1f}%'.format(
                                              ultimaRentaAcumuladaPorCompra) + ' por compra, ' + '{:.0f}'.format(
                                              numeroDiasAnalizados) + ' c, ' + ' std: ' + '{:.1f}%'.format(std),
                                          marker="+")
        ...
        for subgrupo in subgrupos:
            ...
            resultadoPorSubgrupo.plot(kind='line', x='fecha', y='rentaRelativaSP500', ax=ax4,
                                      label=subgrupo + " --> " + '{:.1f}%'.format(
                                          mediaPorCompra) + ' por compra, ' + '{:.0f}'.format(
                                          numeroDiasAnalizados) + " c", marker="+")
        ...
        for subgrupo in subgrupos:
            ...
            if numeroDiasAnalizados > 0:
                ...
                resultadoPorSubgrupo.plot(kind='line', x='fecha', y='rentaAcumuladavsSP500', ax=ax5,
                                          label=subgrupo + " --> " + '{:.1f}%'.format(
                                              ultimaRentaAcumuladavsSP500PorCompra) + ', por compra, ' + '{:.0f}'.format(
                                              numeroDiasAnalizados) + " c", marker="+")
        ...
        for subgrupo in subgrupos:
            ...
            if numeroDiasAnalizados > 0:
                ultimaRentaAcumuladavsSP500ConProbUnoMinima = rentasAcumuladasvsSP500ConProbUnoMinima.iloc[
                    numeroDiasAnalizados - 1]
                ultimaRentaAcumuladavsSP500ConProbUnoMinimaPorCompra = ultimaRentaAcumuladavsSP500ConProbUnoMinima / numeroDiasAnalizados
                resultadoPorSubgrupo.plot(kind='line', x='fecha', y='rentaAcumuladavsSP500ConProbUnoMinima', ax=ax6,
                                          label=subgrupo + " --> " + '{:.1f}%'.format(
                                              ultimaRentaAcumuladavsSP500ConProbUnoMinimaPorCompra) + ' por compra, ' + '{:.0f}'.format(
                                              numeroDiasAnalizados) + " c", marker="+")
        ax6.tick_params(axis='x', labelrotation=20)  # Rota las etiquetas del eje X
        formatter = mdates.DateFormatter("%Y-%m-%d")
        locator = mdates.DayLocator()
        plt.title('Prob MIN=' + '{:.2f}%'.format(probUnoMinima) + ' -> RENTA ACUM vs SP500 -espero ' + str(
            Y) + ' días SOLAPADOS-', fontsize=10)
        plt.xticks(rotation=90, ha='right')
        # plt.show()
...
# -----------------------------------------------------------
def analizar(datosGrandes, datosManejables, X, dfSP500, descripcion):
    # Se obtiene una relación unívoca entre antigüedad y fecha. El valor 0 será el más reciente.
    # Entonces, si quiero ir hacia el futuro, debo restar posiciones del índice.
    # Se aplicará el mismo formato donde se necesite.
    print("ANALIZAR() - " + descripcion)
    fechasAcumuladas = 10000 * datosGrandes['anio'] + 100 * datosGrandes['mes'] + \
                       datosGrandes['dia']
    fechasPorAntiguedad = fechasAcumuladas.unique()

    print("Comprobación de que los datos GRANDES contienen muchas fechas distintas (histórico)...")
    if len(fechasPorAntiguedad) <= 1:
        raise RuntimeError("Los datos GRANDES contienen solo " + str(
            len(fechasPorAntiguedad)) + " fechas. Deberían contener decenas (histórico). Hay algo mal. Saliendo...")
    ...
    for row_index, row in datosMergeados.iterrows():
        ...
        if aniomesdia in listaFechasPorAntiguedad:
            ...
            # Desplazo el índice hacia el futuro. Es decir, restar días en al antiguedad
            indice -= int(X)
            # print("analizar - aniomesdia=" + str(aniomesdia) + " - indice=" + str(indice))
            ...
            if indice >= 0:
                ...
                fila = datosGrandes.loc[
                    (datosGrandes['aniomesdia'] == fechaDesplazada) & (datosGrandes['empresa'] == row['empresa'])]
    ...
    if 'empresa' in datosGrandes and 'empresa' in datosDesplazados:
        ...
        for subgrupo in subgrupos:
            resultadoPorSubgrupo = resultadoAnalisis.loc[resultadoAnalisis['subgrupo'] == subgrupo]
            resultadoPorSubgruposinnan = resultadoPorSubgrupo[
                resultadoPorSubgrupo['rentaRelativaSP500ConProbUnoMinima'].notna()]
...
def calidadSubgrupos(resultadoAnalisis, subgrupos):
    ...
    if not resultadoAnalisis.empty:
        ...
        for subgrupo in subgrupos:
            ...
            nuevoGrupo = [{'subgrupo': subgrupo, 'calidadPonderada': calidadPonderada, 'calidadMediana': calidadMediana,
                           'calidadMediaStd': calidadMediaStd}]
...
# ------------------------------ANÁLISIS BÁSICO (RENTABILIDAD ESPERANDO X DIAS) -----------------------------
print("ANALISIS BASICO - Rentabilidad esperando X=" + str(X) + " días")
# Ordenamos para poder debuguear bien
datosManejables.sort_values(by=['empresa', 'anio', 'mes', 'dia'], ascending=True, inplace=True)

resultadoAnalisis, subgrupos = analizar(datosGrandes, datosManejables, X, dfSP500, "antiguedad_" + str(X))
if resultadoAnalisis.shape[0] == 0:
    raise RuntimeError("El dataframe resultadoAnalisis tiene 0 filas. Esta mal, hay que revisarlo. Saliendo...")
