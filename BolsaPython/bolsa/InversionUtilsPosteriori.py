...
def analizar(datosGrandes, datosManejables, X, dfSP500):
    ...
    if VELA_YF == '1d':
        ...
        fechasAcumuladas = 10000 * datosGrandes['anio'] + 100 * datosGrandes['mes'] + \
                           datosGrandes['dia'] + 0.01 * datosGrandes['hora']
    else:
    ...
    if VELA_YF == '1d':
        ...
        datosMergeados = pd.merge(datosGrandes, datosManejables, how='right',
                                  on=['empresa', 'anio', 'mes', 'dia', 'hora', 'close'])
        datosMergeados['aniomesdiahora'] = 10000 * datosMergeados['anio'] + 100 * datosMergeados['mes'] + \
                                           datosMergeados['dia'] + 0.01 * datosMergeados['hora']
        datosGrandes['aniomesdiahora'] = 10000 * datosGrandes['anio'] + 100 * datosGrandes['mes'] + \
                                         datosGrandes['dia'] + 0.01 * datosGrandes['hora']
    else:
        sys.exit('ERROR: en este análisis no se tratan velas de duración: ' + VELA_YF)
    ...
    for row_index, row in datosMergeados.iterrows():
        if VELA_YF == '1d':
            fecha = row['aniomesdia']
        elif VELA_YF == '1h':
            fecha = row['aniomesdiahora']
        else:
            sys.exit('ERROR: en este análisis no se tratan velas de duración: ' + VELA_YF)

        if fecha in listaFechasPorAntiguedad:
            indice = listaFechasPorAntiguedad.index(fecha)
    ...
    if 'empresa' in datosGrandes and 'empresa' in datosDesplazados:
        if VELA_YF == '1d':
            ...
            datosFuturo = pd.merge(datosGrandes, datosDesplazados, how='right', on=['empresa', 'aniomesdiahora'])
        else:
            sys.exit('ERROR: en este análisis no se tratan velas de duración: ' + VELA_YF)
...
if VELA_YF == '1d':
    columnasManejables = ['empresa', 'antiguedad', 'anio', 'mes', 'dia', 'close', 'TARGET_PREDICHO',
                          'TARGET_PREDICHO_PROB']
elif VELA_YF == '1h':
    columnasManejables = ['empresa', 'antiguedad', 'anio', 'mes', 'dia', 'hora', 'close', 'TARGET_PREDICHO',
                          'TARGET_PREDICHO_PROB']
else:
    sys.exit('ERROR: en este análisis no se tratan velas de duración: ' + VELA_YF)
...
patronGrupoCeroAntCero = "*_GRANDE_0_SG_0_*"
ficherosGrandesCeroGrupo0 = glob.glob(dirDropbox + patronGrupoCeroAntCero)
# Se ordena por fecha, y se toma el más reciente
ficherosGrandesCeroGrupo0.sort(reverse=True)
if not ficherosGrandesCeroGrupo0:
    print("ERROR: NO SE HA ENCONTRADO NINGÚN FICHERO CON EL PATRÓN: " + patronGrupoCeroAntCero)
