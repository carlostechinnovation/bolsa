...
#----------------------INICIO DE H20-------------------------------------------
    ...
    # Datos
    datos = pd.read_csv('inputbolsa2/COMPLETO.csv', sep='|')
    ...
    ds_train, ds_test, ds_validacion = np.split(inputFeaturesyTarget.sample(frac=1),
                                       [int(fraccion_train * len(datos)),
                                        int((fraccion_train + fraccion_test) * len(datos))])
    ...
    # AutoML H2O:
    aml = H2OAutoML(max_models=80, max_runtime_secs=10, seed=247)  # --> CAMBIAR EL TIEMPO
    ...
    ganador_nombreModelo=modelo

    # Predicciones y tratamientos
    test_predicho_h20=modelo.predict(traindf).as_data_frame()
    validac_predicho_h20 = modelo.predict(validaciondf).as_data_frame()

    # Conversión a dataframe pandas
    datatest = {'actual': ds_test['TARGET'], 'Ypredict': test_predicho_h20['predict'].tolist()}
    df_test = pd.DataFrame(datatest, columns=['actual', 'Ypredict'])
    datavalidacion = {'actual': ds_validacion['TARGET'], 'Ypredict': validac_predicho_h20['predict'].tolist()}
    df_validacion = pd.DataFrame(datavalidacion, columns=['actual', 'Ypredict'])
...
#-------------------------------FIN DE H2O----------------------------------
    ...
    if os.path.isfile(path_modelo_predictor_ganador):
        ...
        # Predicciones y tratamientos
        print("Predecir:")
        predict = modelo.predict(inputFeatures_sinnulos)
        ...
        # probabilities
        probs=pd.DataFrame(p['p1'].tolist(), columns=['TARGET_PREDICHO_PROB'])

        # UMBRAL MENOS PROBABLES CON TARGET=1. Cuando el target es 1, se guarda su probabilidad
        probabilidadesEnTargetUnoPeq = probs
